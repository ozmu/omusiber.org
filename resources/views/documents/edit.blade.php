    ...
    <form id="tool-form" name="contact" action="/docs/tool/{{$toolInfo->tool}}/edit" method="POST" novalidate="novalidate" style="margin: 50px 0;">
        ...
        <div class="row form-group over-hidden">
            <div class="col-md-6">
                ...
                <input type="text" id="tool-name" name="tool-name" placeholder="{{$toolInfo->toolName}}" class="form-control required" maxlength="30" required>
            </div>
            <div class="col-md-6">
                ...
                <div class="form-box floated-controls checkbox-table">
                    @foreach($categories as $category)
                        <div class="f-left m-r-1">
                            <input type="checkbox" name="category[]" @if(in_array($category->category,$tool_cats)) checked @endif value="{{$category->category}}" class="labelauty" data-labelauty="{{ $category->categoryName }}">
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="form-group over-hidden">
                    <textarea name="tool-content" data-height="400" data-provide="markdown" class="form-control required">{!! $toolInfo->description !!}</textarea>
                </div>
            </div>
        </div>
